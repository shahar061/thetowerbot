...
/** Starting a new reroll retires every emulator not kept, permanently. The
 *  warning is its own step so it cannot be skimmed past on the way to the
 *  checkboxes. */
export function NewRerollDialog({ open, onClose, run, members, candidates, busy, error, onConfirm }: {
  ...
  const retiring = members.length - keep.length;
  ...
  if (step === "warning" && run) {
    return <ConfirmDialog open={open} onOpenChange={change} tone="warn" title={`Close ${run.name}?`}
      ...
      <p>Starting a new reroll closes {run.name} for good. Emulators you don&apos;t keep playing will be stopped, shut down and removed from the reroll. They can&apos;t be played again.</p>
  ...
  return <ConfirmDialog open={open} onOpenChange={change} title={run ? `Start Reroll #${next}` : "Start a reroll"}
    footer={<><Button variant="outline" onClick={onClose}>Cancel</Button>
      <Button variant={run && retiring ? "destructive" : "default"} disabled={busy || !keep.length && !add.length}
        onClick={() => onConfirm({ keep, add })}>
        {run && retiring ? `Start Reroll #${next} and retire ${retiring}` : `Start Reroll #${next}`}
      </Button></>}>
    ...
    {run && members.length > 0 && <fieldset className="space-y-1.5">
      ...
      {members.map(member => <label key={member.name} className="flex items-center gap-2 rounded-lg border border-border p-2">
        <input type="checkbox" checked={keep.includes(member.name)} disabled={busy}
    ...
    <fieldset className="space-y-1.5">
      ...
      {candidates.map(candidate => <label key={candidate.name} className={cn("flex items-center gap-2 rounded-lg border border-border p-2", !eligible(candidate) && "bg-muted/40 opacity-70")}>
        <input type="checkbox" checked={add.includes(candidate.name)} disabled={busy || !eligible(candidate)}
    ...
    {run && <p className="font-mono text-xs">Keep {keep.length} · Add {add.length} · <span className={cn(retiring && "text-danger")}>Retire {retiring}</span></p>}
  </ConfirmDialog>;
}
