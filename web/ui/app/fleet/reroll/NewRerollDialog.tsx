...
export function NewRerollDialog({ open, onClose, run, members, candidates, busy, error, onConfirm }: {
  ...
  // (a candidate retires, a member leaves the run) without the checkbox
  ...
  const retiring = members.length - chosenKeep.length;
  ...
  return <ConfirmDialog open={open} onOpenChange={change} title={run ? `Start Reroll #${next}` : "Start a reroll"}
    footer={<><Button variant="outline" onClick={onClose}>Cancel</Button>
      <Button variant={run && retiring ? "destructive" : "default"} disabled={busy || !chosenKeep.length && !chosenAdd.length}
    ...
    {run && <p className="font-mono text-xs">Keep {chosenKeep.length} · Add {chosenAdd.length} · <span className={cn(retiring && "text-danger")}>Retire {retiring}</span></p>}
  </ConfirmDialog>;
}
