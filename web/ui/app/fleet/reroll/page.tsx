    ...
    {!!members.length && <SectionCard title="Workers"><div className="grid gap-3 lg:grid-cols-2">{members.map(member => {
      ...
      return <article key={member.name} className="min-w-0 rounded border p-3 text-sm"><div className="flex flex-wrap items-center justify-between gap-2"><div><h2 className="font-semibold">{member.name}</h2><p className="text-muted-foreground">{stateLabel(member.state)} · {member.endpoint}</p></div>{account && <Link href="/" onClick={() => choose(account.key)} className="text-primary underline">Open account {account.account_id}</Link>}</div>
        <dl className="mt-3 grid grid-cols-2 gap-x-4 gap-y-2 sm:grid-cols-3">{([
          ["Verified account ID", member.account_id ?? account?.account_id], ["Milestone", member.milestone], ["Latest completed tier / wave", member.tier != null && member.wave != null ? `${member.tier} / ${member.wave}` : null], ["Run duration (s)", member.run_duration_seconds], ["Best Tier 1 wave", member.best_tier_1_wave], ["Wallet coins", member.wallet_coins], ["Run coins", member.run_coins], ["Wallet gems", member.wallet_gems], ["Wallet stones", member.wallet_stones], ["Wallet medals", member.wallet_medals], ["UW result", member.uw_result], ["Last observation", observed(member.observed_at)],
