...
export function BattleFlow({ battle, catalog, onChange }: {
  ...
  function addPriority(branch: Branch, entry: Phase, id: string, before?: string): void {
    if (!upgrades.some(item => item.id === id)) return;
    const targetIndex = before ? entry.priority_ids.indexOf(before) : -1;
    const ids = entry.priority_ids.filter(value => value !== id);
    ids.splice(targetIndex < 0 ? ids.length : Math.min(targetIndex, ids.length), 0, id);
    editPhase(branch, entry.id, { priority_ids: ids });
  }
  return <section className={`${styles.canvas} space-y-4`} aria-label="Battle flow builder">
    ...
    {battle.mode === "phases" && <>
      ...
      {battle.branches.map(branch => <article key={branch.id} className="space-y-3 rounded-xl border border-primary/30 bg-card/90 p-3" aria-label={`Battle branch ${branch.id}`}>
        ...
        <div className="grid gap-3 md:grid-cols-2">{branch.phases.map(entry => <section key={entry.id} className="space-y-3 rounded-xl border border-border bg-background/40 p-3 text-xs" aria-label={`Wave phase ${entry.id}`}>
          ...
          <button type="button" aria-pressed={entry.emergency_survival} onClick={() => editPhase(branch, entry.id, { emergency_survival: !entry.emergency_survival })}
            className={`rounded-lg border px-3 py-2 text-left ${entry.emergency_survival ? "border-primary bg-primary/10" : "border-border"}`}>
            Survival interrupt <span className="font-semibold">{entry.emergency_survival ? "ON" : "OFF"}</span>
          </button>
          <ol data-testid={`battle-path-${entry.id}`} aria-label={`Priority blocks for ${entry.id}`} onDragOver={event => event.preventDefault()}
            onDrop={event => { event.preventDefault(); if (dragged) addPriority(branch, entry, dragged); setDragged(null); }}
            className={`${styles.drop} min-h-14 space-y-2`}>{entry.priority_ids.map((id, index) => <li key={id}
              data-testid={`battle-priority-${entry.id}-${id}`} draggable onDragStart={event => { event.stopPropagation(); setDragged(id); }}
              onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); event.stopPropagation(); if (dragged && dragged !== id) addPriority(branch, entry, dragged, id); setDragged(null); }} onDragEnd={() => setDragged(null)}
              className={`${styles.block} ${styles[(catalog.find(item => item.id === id)?.category ?? "UTILITY").toLowerCase()]} flex items-center gap-1 px-3 py-2`}>
              <span className="flex-1">{names.get(id) ?? id}</span><button type="button" disabled={index === 0} aria-label={`Move ${names.get(id) ?? id} up in ${entry.id}`} onClick={() => { const ids = [...entry.priority_ids]; [ids[index - 1], ids[index]] = [ids[index], ids[index - 1]]; editPhase(branch, entry.id, { priority_ids: ids }); }} className="rounded border border-border px-1 disabled:opacity-40">↑</button><button type="button" onClick={() => editPhase(branch, entry.id, { priority_ids: entry.priority_ids.filter(value => value !== id), weights: Object.fromEntries(Object.entries(entry.weights).filter(([key]) => key !== id)) })} aria-label={`Remove ${names.get(id) ?? id} from ${entry.id}`} className="rounded border border-border px-1">×</button>
          ...
          <div aria-label={`Upgrade palette for ${entry.id}`} className="rounded-xl border border-border bg-card p-2">
            <p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Drag upgrade blocks into this phase</p>
            <div className={`${styles.palette} grid max-h-32 gap-2 sm:grid-cols-2`}>{upgrades.filter(item => !entry.priority_ids.includes(item.id)).map(item => <div key={item.id} draggable data-testid={`battle-palette-${entry.id}-${item.id}`}
              onDragStart={() => setDragged(item.id)} onDragEnd={() => setDragged(null)} className={`${styles.block} ${styles[item.category.toLowerCase()]} flex items-center justify-between gap-1 px-2 py-2`}>
              <span>{item.name}</span><button type="button" aria-label={`Add ${item.name} to ${entry.id}`} onClick={() => addPriority(branch, entry, item.id)} className="rounded border border-border px-2 py-1">+</button>
            </div>)}</div>
          </div>
