...
export default function ArchivesPage() {
  ...
  const displayedNumbers = run !== "all" && !numbers.includes(Number(run))
    ? [...numbers, Number(run)].sort((a, b) => b - a)
    : numbers;
  const showFilter = displayedNumbers.length > 0 || run !== "all";
  return <div className="mx-auto flex max-w-5xl flex-col gap-4">
    ...
    {showFilter && <label className="flex items-center gap-2 text-sm text-muted-foreground">Reroll
      <select aria-label="Reroll" value={run} onChange={event => setRun(event.target.value)} className="rounded-md border bg-background px-2 py-1 text-foreground">
        ...
        {displayedNumbers.map(number => <option key={number} value={String(number)}>Reroll #{number}</option>)}
      </select></label>}
    {archived.length === 0 && run === "all" ? <SectionCard title="No archived accounts">
    ...
    </SectionCard> : archived.length === 0 && run !== "all" ? <SectionCard title={`No archived accounts in Reroll #${run}`}>
      <p className="text-sm">Accounts from this reroll are still playing, or the reroll has no retired emulators yet.</p>
      <button onClick={() => setRun("all")} className="mt-3 inline-block text-sm text-primary underline">Show all archives</button>
